  ...
  try {
    auto rd = get_random_engine();

        {
          ...
          ); test.execute( AckReceived { Wrap32 { isn + 1 } }.with_win( 1000 ) ); test.execute( ExpectSeqno { isn
          + 1 } ); test.execute( ExpectSeqnosInFlight { 0 } ); test.execute( Push( "abc" ) ); test.execute(
          ExpectMessage {}.with_payload_size( 3 ).with_data( "abc" ).with_seqno( isn + 1 ) ); test.execute( Tick {
          rto - 5 } ); test.execute( ExpectNoSegment {} ); test.execute( Push( "def" ) ); test.execute(
          ExpectMessage {}.with_payload_size( 3 ).with_data( "def" ) ); test.execute( Tick { 6 } ); test.execute(
          ExpectMessage {}.with_payload_size( 3 ).with_data( "abc" ).with_seqno( isn + 1 ) ); test.execute(
          ExpectNoSegment {} );
        }

        {
          ...
          ExpectMessage {}.with_payload_size( 3 ).with_data( "abc" ).with_seqno( isn + 1 ) ); test.execute( Tick {
          rto - 5 } ); test.execute( ExpectNoSegment {} ); test.execute( Push( "def" ) ); test.execute(
          ExpectMessage {}.with_payload_size( 3 ).with_data( "def" ) ); test.execute( Tick { 200 } );
...

        {
          ...
          + 1 } ); test.execute( ExpectSeqnosInFlight { 0 } ); test.execute( Push( "abc" ) ); test.execute(
          ExpectMessage {}.with_payload_size( 3 ).with_data( "abc" ).with_seqno( isn + 1 ) ); test.execute( Tick {
          rto - 5 } ); test.execute( Push( "def" ) ); test.execute( ExpectMessage {}.with_payload_size( 3
          ).with_data( "def" ).with_seqno( isn + 4 ) ); test.execute( AckReceived { Wrap32 { isn + 4 } }.with_win(
          1000 ) ); test.execute( Tick { rto - 1 } ); test.execute( ExpectNoSegment {} ); test.execute( Tick { 2 }
...

        {
          ...
          ExpectMessage {}.with_payload_size( 3 ).with_data( "abc" ).with_seqno( isn + 1 ) ); test.execute( Tick {
          rto - 5 } ); test.execute( Push( "def" ) ); test.execute( ExpectMessage {}.with_payload_size( 3
          ).with_data( "def" ).with_seqno( isn + 4 ) ); test.execute( AckReceived { Wrap32 { isn + 1 } }.with_win(
          1000 ) ); test.execute( Tick { 6 } ); test.execute( ExpectMessage {}.with_payload_size( 3 ).with_data(
          "abc" ).with_seqno( isn + 1 ) ); test.execute( ExpectNoSegment {} ); test.execute( Tick { rto * 2 - 5 }
          ); test.execute( ExpectNoSegment {} ); test.execute( Tick { 8 } ); test.execute( ExpectMessage
          {}.with_payload_size( 3 ).with_data( "abc" ).with_seqno( isn + 1 ) ); test.execute( ExpectNoSegment {}
          );
        }

        {
          ...
          ExpectMessage {}.with_payload_size( 3 ).with_data( "abc" ).with_seqno( isn + 1 ) ); test.execute( Tick {
          rto - 5 } ); test.execute( Push( "def" ) ); test.execute( ExpectMessage {}.with_payload_size( 3
          ).with_data( "def" ).with_seqno( isn + 4 ) ); test.execute( Push( "ghi" ) ); test.execute( ExpectMessage
          {}.with_payload_size( 3 ).with_data( "ghi" ).with_seqno( isn + 7 ) ); test.execute( AckReceived { Wrap32
          { isn + 1 } }.with_win( 1000 ) ); test.execute( Tick { 6 } ); test.execute( ExpectMessage
          {}.with_payload_size( 3 ).with_data( "abc" ).with_seqno( isn + 1 ) ); test.execute( ExpectNoSegment {}
          ); test.execute( Tick { rto * 2 - 5 } ); test.execute( ExpectNoSegment {} ); test.execute( Tick { 5 } );
          test.execute( ExpectMessage {}.with_payload_size( 3 ).with_data( "abc" ).with_seqno( isn + 1 ) );
          test.execute( ExpectNoSegment {} );
        ...
        }
        {
          ...
          test.execute( Push {} );
          test.execute( ExpectMessage {}.with_no_flags().with_syn( true ).with_payload_size( 0 ).with_seqno( isn )
          ); test.execute( AckReceived { Wrap32 { isn + 1 } }.with_win( window_size ) ); test.execute( ExpectSeqno
          { isn + 1 } ); test.execute( ExpectSeqnosInFlight { 0 } ); test.execute( Push { bigstring } );

          for ( unsigned int i = 0; i + TCPConfig::MAX_PAYLOAD_SIZE < min( bigstring.size(), window_size );
                i += TCPConfig::MAX_PAYLOAD_SIZE ) {
            const size_t expected_size = min( TCPConfig::MAX_PAYLOAD_SIZE, min( bigstring.size(), window_size ) -
            i ); test.execute( ExpectMessage {}
                            .with_no_flags()
...

        {
          ...
          + 1 } ); test.execute( ExpectSeqnosInFlight { 0 } ); test.execute( Push { "abc" }.with_close() );
          test.execute(
